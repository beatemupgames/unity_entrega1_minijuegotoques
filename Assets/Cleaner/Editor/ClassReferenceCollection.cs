...
namespace AssetClean
{
	public class ClassReferenceCollection : IReferenceCollection
	{
		...
		List<System.Type> CollectionAllClasses()
		{
			...
			// Ruta absoluta para Assembly-CSharp.dll
			string runtimeAssembly = Path.GetFullPath("Library/ScriptAssemblies/Assembly-CSharp.dll");
			if (File.Exists(runtimeAssembly))
				alltypes.AddRange(Assembly.LoadFile(runtimeAssembly).GetTypes());

			// Ruta absoluta para Assembly-CSharp-Editor.dll (solo si se guarda código de editor)
			if (isSaveEditorCode)
			{
				string editorAssembly = Path.GetFullPath("Library/ScriptAssemblies/Assembly-CSharp-Editor.dll");
				if (File.Exists(editorAssembly))
					alltypes.AddRange(Assembly.LoadFile(editorAssembly).GetTypes());
			}
